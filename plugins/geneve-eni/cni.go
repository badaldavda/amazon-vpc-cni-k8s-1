...
import (
	...
	"github.com/aws/amazon-vpc-cni-k8s/plugins/geneve-eni/driver"
...
func add(args *skel.CmdArgs, cniTypes typeswrapper.CNITYPES, grpcClient grpcwrapper.GRPC,
	...
	log.Infof("Received add network response for pod %s namespace %s container %s: %s, table %d, external-SNAT: %v, vpcCIDR: %v, CGWIP: %v, vni: %v, dstPort: %v",
		string(k8sArgs.K8S_POD_NAME), string(k8sArgs.K8S_POD_NAMESPACE), string(k8sArgs.K8S_POD_INFRA_CONTAINER_ID),
		r.IPv4Addr, r.DeviceNumber, r.UseExternalSNAT, r.VPCcidrs, r.CGWIP, r.VNI, r.DstPort)
	...
	err = driverClient.SetupNS(hostVethName, args.IfName, geneveName, brName, args.Netns, addr, int(r.DeviceNumber), r.VPCcidrs, r.UseExternalSNAT,
		r.CGWIP, r.VNI, r.DstPort)
...
// generateGenveName returns a name to be used on the host-side veth device.
func generateGeneveName(prefix, namespace, podname string) string {
	h := sha1.New()
	h.Write([]byte(fmt.Sprintf("%s.%s", namespace, podname)))
	return fmt.Sprintf("%s%s", "gen", hex.EncodeToString(h.Sum(nil))[:11])
}

// generateBrName returns a name to be used on the host-side veth device.
func generateBrName(prefix, namespace, podname string) string {
	h := sha1.New()
	h.Write([]byte(fmt.Sprintf("%s.%s", namespace, podname)))
	return fmt.Sprintf("%s%s", "br", hex.EncodeToString(h.Sum(nil))[:11])
}

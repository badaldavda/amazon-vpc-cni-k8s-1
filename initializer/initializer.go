...
const (
	defaultLogFilePath     = "/host/var/log/aws-routed-eni/initializer.log"
	version                = "0.1.0"
	defaultInitializerName = "aws-vpc-eni.initializer.kubernetes.io"
	defaultConfigmap       = "aws-vpc-eni-initializer"
...
func main() {
	...
	log.Infof("Starting %s for %s: %s", initializerName, configmap, version)
	...
	if err != nil {
		log.Errorf("Failed to retrieve configmap: %v", err)
		panic(err.Error())
	}

	c, err := configmapToConfig(cm)
	if err != nil {
		log.Errorf("Failed to parse configmap: %v", err)
		panic(err.Error())
	}
...
func initializeDeployment(deployment *v1beta1.Deployment, c *config, clientset *kubernetes.Clientset) error {
	if deployment.ObjectMeta.GetInitializers() != nil {
		...
		log.Debugf("Received trigger on initializer %s, vs mine is %s",
			pendingInitializers[0].Name, initializerName)

		if initializerName == pendingInitializers[0].Name {
			log.Infof("Initializing deployment: %s", deployment.Name)

			o, err := runtime.NewScheme().DeepCopy(deployment)
			if err != nil {
				log.Errorf("Failed to DeepCopy :%v", err)
				return err
			}
			...
			if len(pendingInitializers) == 1 {
				log.Debugf("Completed all initializers")
				...
				log.Debugf("There are %d un-processed initializers: %s ...", len(initializedDeployment.ObjectMeta.Initializers.Pending),
					initializedDeployment.ObjectMeta.Initializers.Pending[0].Name)
			}

			// TODO : neet to check if hostNetwork=true
			...
			if err != nil {
				log.Errorf("Failed to Marshal old deployment: %v", err)
				return err
			}
			log.Debugf("Old deployment received: %v", deployment)

			newData, err := json.Marshal(initializedDeployment)
			if err != nil {
				log.Errorf("Failed to Marshal initializedDeployment: %v", err)
				return err
			}
			log.Debugf("New deployment generated: %v", initializedDeployment)

			patchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldData, newData, v1beta1.Deployment{})
			if err != nil {
				log.Errorf("Failed to patch initializer: %v", err)
				return err
			}

			log.Debugf("Patched deployment generated: %s", patchBytes)
